use std::{ffi::{CStr, CString}, env::consts::DLL_EXTENSION, ptr, mem, error};
use winapi::{um::{tlhelp32::{PROCESSENTRY32, Process32First, Process32Next, CreateToolhelp32Snapshot, TH32CS_SNAPPROCESS}, libloaderapi::{GetModuleHandleA, GetProcAddress}, winnt::{PROCESS_CREATE_THREAD, PROCESS_VM_OPERATION, PROCESS_VM_WRITE, MEM_RESERVE, MEM_COMMIT, PAGE_READWRITE}, handleapi::CloseHandle, processthreadsapi::{OpenProcess, CreateRemoteThread}, memoryapi::{VirtualAllocEx, WriteProcessMemory}}, shared::minwindef::{FALSE, LPVOID, DWORD}};
use thiserror::Error;
...
fn init() -> Result<(), Box<dyn error::Error>> {
  println!("Waiting for Genshin Impact or Honkai: Star Rail to start");
  
  let mut process_id = 0;
  
  loop {
    for process_name in vec!["GenshinImpact.exe", "YuanShen.exe", "StarRail.exe"] {
      let id = unsafe { get_process_id_by_name(process_name)? };
